...
import { CONTENT_TEST_CASES, ContentTestCase } from '../../data/datasets'
import { ChatbotApiService, loadChatbotApiConfig, probeChatbotApi } from '../../service/chatbot'

const contentConfig = loadChatbotApiConfig()
const hasRequiredEnv = !!contentConfig.threadId
let canRunContentSuite = hasRequiredEnv
let skipReason = 'Set CHATBOT_THREAD_ID in .env to run content tests'

const apiService = new ChatbotApiService(contentConfig)

test.beforeAll(async () => {
  if (!hasRequiredEnv) return
  const probe = await probeChatbotApi(contentConfig)
  if (!probe.ok) {
    canRunContentSuite = false
    skipReason =
      probe.reason ??
      `Chatbot API is unavailable for content suite (status ${probe.status}). Check CHATBOT_THREAD_ID / API health.`
  }
})
...
for (const tc of singleTurnCases) {
  test(`[${tc.id}] ${tc.description}`, async () => {
    test.skip(!canRunContentSuite, skipReason)
